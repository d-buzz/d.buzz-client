...
export const getUrls = (text) => {
  const regexUrls =
    /(http|ftp|https):\/\/([\w_-]+(?:(?:\.[\w_-]+)+))([\w.,@?^=%&:/~+#-]*[/w@?^=%&/~+#-(a-z)(A-Z)(0-9)])?/gm
...
export const calculatePayout = (data) => {
  ...
  if (is_paidout) {
    ...
    payout =
      parseFloat(`${total_payout_value}`.replace("HBD")) +
      parseFloat(`${curator_payout_value}`.replace("HBD")) +
      parseFloat(`${pending_payout_value}`.replace("HBD"))
  }
...
export const errorMessageComposer = (
  type = null,
  errorCode = 0,
  timeLeft = 0
) => {
  let errorMessage =
    "Transaction broadcast failure for unknown reason, please contact the administrator"
  ...
  if (errorCode === -32000) {
    errorMessage +=
      ", you have insufficient resource credit to make this transaction, please consider retrying after recharge or after powering up hive"
  }

  if (errorCode === -32001) {
    errorMessage +=
      ", votes evaluating for post/comment that is paid out is forbidden."
  }
...
export const signOnHiveonboard = () => {
  const win = window.open(
    "https://hiveonboard.com/create-account?ref=dbuzz&redirect_url=https://d.buzz/#/?status=success",
    "_blank"
  )
  win.blur()
}
...
export const redirectOldLinks = () => {
  ...
  if (window.location.hash) {
    ...
    const regexForOldPostLinks =
      /#\/@([A-Za-z0-9-]+\.?[A-Za-z0-9-]+)\/c\/[a-zA-Z0-9]+/gi

    if (
      regexForOldProfileLinks.test(window.location.hash) ||
      regexForOldPostLinks.test(window.location.hash)
    ) {
...
export const getTheme = () => {
  const theme = JSON.parse(localStorage.getItem("customUserData"))?.settings
    ?.theme
  let mode = ""
...
export const parseUrls = (c) => {
  return (
    c.match(
      /((http|ftp|https):\/\/)?([\w_-]+(?:(?:\.[\w_-])+))+([a-zA-Z]*[a-zA-Z]){1}?(\/+[\w.,@?^=%&:/~+!#-$-']*)*/gm
    ) || []
  )
}

export const getCurrentTimePart = () =>
  Math.floor((new Date().getHours() / 24) * 8) + 1
...
export const calculateAverageRanking = (users) => {
  ...
  users.forEach((userList, systemIndex) => {
    if (userList.length === 10) {
      userList.forEach((user, userIndex) => {
        ...
        userRankings[author].totalRank +=
          ((11 - rank) / 55) * (100 + ranking_importance)
  ...
  if (averageRankings.length > 0) {
    for (let i = 0; i < 10; i++) {
      top10Users.push({
        rank: i + 1,
        author: Object.values(averageRankings)[i].author,
        averageRank: Object.values(averageRankings)[i].averageRank,
      })
    }
  } else top10Users = []
  return top10Users
