...
const render = (content, markdownClass, assetClass, scrollIndex, recomputeRowIndex, classes) => {  
  ...
  if(content.includes(':youtube:')) {
    ...
    // render web images links
      .replace(/(\[\S+)|(\(\S+)|(https?:\/\/.*\.(?:png|jpg|gif|jpeg|bmp))/gi, n => checkForValidImage(n) && JSON.parse(localStorage.getItem('settings'))?.showImagesStatus === 'enabled' ? `![](${n})` : n)
    // render IPFS images
      .replace(/(\[\S+)|(\(\S+)|(?:https?:\/\/(?:ipfs\.io\/ipfs\/[a-zA-Z0-9]+))/gi, n => checkForValidImage(n) && JSON.parse(localStorage.getItem('settings'))?.showImagesStatus === 'enabled' ? `![](${n})` : n)
...
const Renderer = React.memo((props) => {
  ...
  if(JSON.parse(localStorage.getItem('settings'))?.videoEmbedsStatus === 'enabled') {
    links.forEach((link) => {
      try {
        link = link.replace(/&amp;/g, '&')
