...
const render = (content, markdownClass, assetClass, minifyAssets, scrollIndex, recomputeRowIndex, classes, videoEmbeds, soundEmbeds, twitterEmbeds, buzzImages, buzzVideos, onImageLoad) => {
  ...
  if (content.includes(':dbuzz-images-container:')) {
    ...
    const removeLastSpecialCharacters = (n) => {
      // Remove all trailing commas, periods, or question marks not followed by an equal sign
      return n.replace(/(?<![?]=)[,.?]+$/, '')
    }

    const getLastSpecialCharacters = (n) => {
      // Match and return all trailing commas, periods, or question marks not followed by an equal sign
      const match = n.match(/(?<![?]=)[,.?]+$/)
      return match ? match[0] : ''
    }

    const hasConsecutivePeriods = (n) => {
      return /\.{2,}/.test(n)
    }
    ...
    // // render all urls
      .replace(/("\S+)|(\[\S+)|(\(\S+)|(@\S+)|(#\S+)|((http|ftp|https):\/\/)?([\w_-]+(?:(?:\.[\w_-])+))+([a-zA-Z]*[a-zA-Z]){1}?([\w.,@?^=%&:/~+#!-$-]+)?(\/+[\w.,@?^=%&:/~+#!-$-]*)*/gi, n => {
        ...
        const cleanUrlName = removeLastSpecialCharacters(n)
        ...
        const lastCharacter = getLastSpecialCharacters(n)

        // Check for URLs with multiple consecutive periods
        if (hasConsecutivePeriods(cleanUrlName)) {
          return cleanUrlName + lastCharacter
        }
