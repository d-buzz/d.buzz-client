...
const LiteReplyList = (props) => {
  ...
  const RenderReplies = ({ reply, treeHistory }) => {
    ...
    const [replyCount] = useState(children?.length)
    ...
    let replies = reply.children || []

    if(postType === "HivePost") {
    ...
    return (
      <React.Fragment>
            ...
            <Row style={{ paddingBottom: 0, marginBottom: 0 }}>
              <Col xs="auto" style={{ paddingRight: 0 }} onClick={handleOpenContent}>
                <div className={classes.left}>
                  <Avatar avatarUrl={getIpfsLink(author?.profile?.images?.avatar)} />
                  {replies.length !== 0 && (
              ...
              <Col>
                <div className={classes.right}>
                  <div className={classes.content}>
                    ...
                    >
                      {postType === "HivePost" ? author?.profile?.username : author?.profile?.name || author?.profile?.did}
                    </Link>
        ...
        {replies.length !== 0 && (
          <React.Fragment>
            {replies.map((reply) => (
              <RenderReplies key={reply.parent_permlink} reply={reply} treeHistory={`${treeHistory}|${reply.parent_permlink}`}/>
            ))}
          </React.Fragment>
        )}
      </React.Fragment>
...
export default connect(mapStateToProps, mapDispatchToProps)(LiteReplyList)
